...
> 这一篇对应官方 Reference 的 Data 一章。前五篇把接口和安全做完了，但数据一直是存在内存里的：应用一重启，新增的活动就没了。这一篇解决持久化，并且要把两个绕不开的问题讲透——**N+1 查询**和**事务失效**。这两件事不会让程序报错，只会让它在数据量上来之后莫名其妙地变慢、或者改了数据却存不进去。
