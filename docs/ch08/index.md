...
第 7 篇让应用能主动去调别人的接口。这一篇反过来处理“别人要通知我”和“我要通知别人”的场景：活动上新了要广播给在线的浏览器，用户提交了内容要异步去建索引、发通知，这些都不该卡在 HTTP 请求里做。

这一篇与前面几篇有个根本不同：**Kafka 和 RabbitMQ 都需要外部中间件**。所以本篇的代码单独放一个工程 `activity-server-messaging`（端口 8082），而不是塞进 `activity-server`。理由很实际——`activity-server` 那 58 个测试能在任何一台机器上跑起来，一个中间件都不需要；把需要 broker 的代码混进去，这套“零依赖测试”就没了。
